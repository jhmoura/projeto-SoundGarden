...
const ListarEventos = async () => {
    ...
    const htmlEventos = eventos.map((evento, index)=> {
        ...
        <td>${dataEvento.getUTCDate()}/${dataEvento.getUTCMonth()}/${dataEvento.getUTCFullYear()} ${dataEvento.getHours()}:${dataEvento.getMinutes()}</td>
        <td>${evento.name}</td>
...
const eventos = ListarEventos()
    .then((resp)=>{
        console.log(resp);
        table.innerHTML += resp.slice(0,100);
    })
